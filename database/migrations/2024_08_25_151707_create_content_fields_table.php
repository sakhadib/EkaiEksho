<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('content_fields', function (Blueprint $table) {
            $table->id();
            $table->string('FieldName');
            $table->string('FieldDescription')->nullable();
            $table->string('FieldType')->nullable();
            $table->string('FieldIcon')->nullable();
            $table->string('FieldColor')->nullable();
            $table->string('FieldSlug')->nullable();
            $table->string('FieldStatus')->nullable();
            $table->string('FieldVisibility')->nullable();
            $table->integer('FieldOrder')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('content_fields');
    }
};
